...
class EditSection extends Component {
  ...
  this.state = { 
    ...
    componentDidMount() {
        ...
        AsyncStorage.getItem('editSecId').then((val) => this.setState({ editSecId: val })).done();
        ...
        backpress=()=>{
           ...
           getDetails(userid,colId){
            var json=JSON.stringify({
                "UserID":userid,"Action_For":"section","PK_ID":colId
              });
              console.warn(json+"")
              fetch("http://162.250.120.20:444/Login/SectionCollectionGet",
                ...
                .then((responseJson) => {
                    ...
                    console.warn("edit Section get service")
          ...
          CheckConnectivity1(){    
            NetInfo.fetch().then(state => {
              ...
              if(state.isConnected==true){
                {this.exploredata(this.state.getuserid,this.state.title,this.state.dropdownText,this.state.descdata,this.state.getcol_Id,this.state.getsec_Id)}
            }else{
                alert('No Internet connection.Make sure that Mobile data or Wifi is turned on,then try again.')
            }
           ...
           exploredata(userid,title,privacy,desc,colid,secId){
              ...
              fetch("http://162.250.120.20:444/Login/SectionCollectionAdd",
                ...
                .then((responseJson) => {
                    ...
                    if(responseJson[0].msg=='Success'){
                     {this.props.navigation.navigate('collectionDetail',{'collId':this.state.getcol_Id})}   
                    }
        ...
        return (
            <SafeAreaView style={{ flex: 2,backgroundColor:'#fff' }}>
                <View style={styles.headerRow}>
                      {/* <View style={styles.headerRow}> */}
                      <Text style={styles.heading}>Edit Section</Text>
                     <TouchableOpacity
